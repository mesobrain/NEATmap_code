...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            # 注意创建的stage和论文图中有差异
            # 这里的stage不包含stage的patch_merging层，包含的是下个stage的
        ...
        # self.prob = nn.Conv3d(window_size*embed_dim, num_classes, kernel_size=1, stride=1, padding=0, bias=False)
        # self.softmax = nn.Softmax(dim=1)
...
def swin_base_patch4_window8_256(in_channels: int = 1, num_classes: int = 1000, **kwargs):
    # trained ImageNet-1K
